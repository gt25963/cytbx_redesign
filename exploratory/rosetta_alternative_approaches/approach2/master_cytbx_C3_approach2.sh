...
#SBATCH --job-name=approach2_C3
#SBATCH --nodes=1
#SBATCH --time=12:00:00
#SBATCH --mem=8GB
#SBATCH --output=/scratch/b5ae/mvg2713124.b5ae/cytbx_pipeline/logs/approach2_C3_%j.out
#SBATCH --account=brics.b5ae

# CytbX Oligomeric Redesign Pipeline — RQ1 — C3 trimer (same as the pipeline for the dimer, but replaced with trimer values where appropriate)
...
master_folder="CytbX_C3_approach2"
...
# STARTING STRUCTURE 
initial_structure="/scratch/b5ae/mvg2713124.b5ae/cytbx_pipeline/approach2/C3/top3_relaxed_holo.pdb"

# OLIGOMER PARAMETERS 
oligomeric_state=3
ligand=HEM
num_ligands=6 # 6 becasuse trimer
...
for i in $(seq 1 $number_of_iterations); do
  ...
  echo "step 4b: running ESM3 on all sequences at $(timestamp)"
...
EOF
  ...
  if [ "${i}" -lt "${number_of_iterations}" ]; then
    ...
    "${biopython_python}" "${work_directory}/scripts/build_oligomer_cif.py" \
      "/scratch/b5ae/mvg2713124.b5ae/cytbx_pipeline/approach2/C3/top3_relaxed_holo.pdb" \
      "${top_seq}" \
