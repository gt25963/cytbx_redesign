...
# LigandMPNN -> Boltz-2 (all) + ESM3 (all) -> AF3 + Chai-1 (top 50)
...
for i in $(seq 5 $number_of_iterations); do
    ...
    # STEP 3: Convert FASTA to Boltz YAML
    ...
    # STEP 4: Boltz + ESM3 Run in Parallel
    echo "step 4a: running Boltz on all sequences at $(timestamp)"
    ...
    # STEP 5: Score Boltz Outputs
    echo "step 5: scoring Boltz-2 outputs at $(timestamp)"

    ## feed this script into biopython via a heredoc so the bash variables above get substituted into the code before it runs
    "${biopython_python}" - << EOF
...
EOF
    ...
    if ! ls "${work_directory}/logs/af3_cycle${i}"*.out 2>/dev/null | grep -q "." || \
       grep -r "permission denied" "${work_directory}/logs/af3_cycle${i}"*.out 2>/dev/null | grep -q "weights"; then
        echo "Error: AF3 unavailable - skipping AF3 for cycle ${i}"
        af3_failed=1
    fi
    ...
    if [ "${af3_failed}" -eq 0 ]; then
        while true; do
            ...
            if [ "${af3_wait}" -ge 10800 ]; then
                echo "Error: AF3 timed out - skipping AF3 for cycle ${i}"
                af3_failed=1
                break
            fi
