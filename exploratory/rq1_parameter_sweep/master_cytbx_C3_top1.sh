...
# Again basically everything is the same as master_cytbx_C2_0.2.sh except this is for the top1 pose version. 
...
# STARTING STRUCTURE
initial_structure="/scratch/b5ae/mvg2713124.b5ae/cytbx_pipeline/prescreening/oligomer_screen/C3/holo/top1_holo.pdb"
...
EOF
    ...
    if [ "${i}" -lt "${number_of_iterations}" ]; then
        ...
        "${biopython_python}" "${work_directory}/scripts/build_oligomer_cif.py" \
            "${work_directory}/prescreening/oligomer_screen/C3/holo/top3_holo.pdb" \
            "${top_seq}" \
