...
#Q8 version of the chai array script - block size is fixed rather than detected from the next header, since Q8 candidates always have exactly 4 records (protein + Hem1 + Hem2 + Q8)
ids_list=$1
chai_input_fa=$2
output_base_dir=$3
#Each array task picks one candidate id from the ids list, by line number
...
# match any header ending in _id<N>_chain1
...
# extract just this candidate's chain block into its own per-task fasta
...
conda activate chai
# Run chai prediction on this single candidate's extracted fasta
