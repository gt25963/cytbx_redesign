...
cofactor="Q8"
design_source="${work_directory}/rq2/master_pipeline/RQ2_Q8/cycle_5/seed_id37orig63orig109/outputs"
input_fasta="${design_source}/seqs/holo_Q8_surface_packed_18_1_packed_104_1_packed_63_1_packed_109_1.fa"
clash_csv="${work_directory}/rq2/master_pipeline/RQ2_Q8/cycle_5/clash_results_cycle5.csv"
# --------------------------------------------------------------------
...
with open(clash_csv) as f:
    for row in csv.DictReader(f):
        if str(row.get("overall_clash_free")).strip().lower() == "true":
            matches = re.findall(r'packed_(\d+)_', row["pdb_file"])
            if matches:
                keep.add(int(matches[-1]))
