#!/usr/bin/env bash
# CytbX RQ2 Cofactor-Swap Scoring Pipeline - Q8
# Again same as the pipeline for RQ1 and RQ2 FMN, except where needed adapted for Q8.
# Both haems retained, Q8 = surface-binding addition. 
## Shifts cofactor_chain_index from 2 (FMN) to 3 (Q8).
## Metric of interest is the surface pocket binding pair => [0][3].
## chain order: A=protein(0), B=HEM(1, Hem1), C=HEM(2, Hem2), D=Q8(3).
set -u
...
# RQ2 INPUTS
cofactor="Q8"
design_source="${work_directory}/rq2/design/Q8_pocket/cycle1"
input_fasta="${design_source}/clashfree_input.fa"
clash_csv="${design_source}/clash_check_q8.csv"

ligand_spec="HEM:2,UQ8:1" ## both retained haems + surface quinone (UQ8 CCD), Boltz builds by CCD
boltz_samples=2
top_n_for_chai_af3=50
protein_chain_count=1
cofactor_chain_index=3 ## A=0 protein, B=1 HEM_B, C=2 HEM_C, D=3 Q8
i=2
...
echo "building clash-free FASTA at $(timestamp)"
...
# SCORE BOLTZ OUTPUTS
echo "step 4a: scoring Boltz-2 at $(timestamp)"
"${biopython_python}" - << EOF
import json, csv, glob, os
ci = ${cofactor_chain_index}     
pi = 0
...
"${biopython_python}" "${work_directory}/scripts/fasta_to_af3_q8.py" \
    "${chai_input_path}/top_sequences.fa" "${af3_input_path}"
