...
for state in "${oligomeric_states[@]}"; do
    mkdir -p "${screen_directory}/C${state}/rpxdock"
...
for state in "${oligomeric_states[@]}"; do
    ...
    sbatch "${work_directory}/submit_rpxdock.sh" \
        "${cytbx_monomer}" \
        "${screen_directory}/C${state}/rpxdock" \
        "${state}"
done

# wait for all RPXDock outputs
for state in "${oligomeric_states[@]}"; do
    while true; do
...
for state in "${oligomeric_states[@]}"; do
    for j in $(seq 1 $rpxdock_top_n); do
        ...
        echo "C${state},top${j},${top_boltz},${top_esm3},${top_combined}" \
            >> "${trajectory_path}/screen_scores.csv"
    done
done

sort -t, -k5,5 -n -r "${trajectory_path}/screen_scores.csv" \
    -o "${trajectory_path}/screen_scores.csv"

best_state=$(awk -F',' 'NR==2 {print $1}' "${trajectory_path}/screen_scores.csv")
