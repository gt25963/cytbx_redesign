...
# FIXED version - see superseded/mpnn_to_boltz2.sh for the original issues: 
## That version defaulted ligand chain_start to "B", which collides with an actual protein chain letter for any multi-chain state (C2 dimer = chains A,B; C3 trimer = chains A,B,C) unless -c was passed correctly on every call. 
## Fix here: ligand chain start is computed automatically from the actual chain count of each design (auto_start_ascii), and an explicit -c override that still collides now warns and auto-bumps rather than silently producing a broken YAML.

set -euo pipefail
ligand_ccd="HEM"
outdir="boltz2_inputs"
chain_start="" ## empty = auto-derive from protein chain count, no hardcoded collisiion risk
if [ $# -lt 1 ]; then
...
awk -v outdir="$outdir" -v ligand_ccd="$ligand_ccd" -v chain_start_req="$chain_start" '
  ...
  function write_yaml() {
    ...
    # FIX: fresh per design - ligand chain always start right after the last protein chain letter 
        auto_start_ascii = 64 + nchain + 1
    if (chain_start_req != "") {
        req_ascii = index("ABCDEFGHIJKLMNOPQRSTUVWXYZ", toupper(chain_start_req)) + 64
        if (req_ascii <= 64 + nchain) {
            print "WARNING: -c " chain_start_req " collides with protein chains, auto-bumped to " sprintf("%c", auto_start_ascii) > "/dev/stderr"
