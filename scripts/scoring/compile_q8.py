...
RQ2_Q8 = f"{WORK}/rq2/master_pipeline/RQ2_Q8"
cycle = sys.argv[1]
CYCLE_DIR = f"{RQ2_Q8}/cycle_{cycle}"
TRAJ = f"{RQ2_Q8}/design_trajectory/cycle_{cycle}"
...
# Load top50 mapping
top50 = []
with open(f"{TRAJ}/top50_Q8_cycle{cycle}.csv") as f:
...
for npz in glob.glob(f"{CYCLE_DIR}/chai/outputs/*/scores.model_idx_*.npz"):
    ...
    if ci_uq8 < mat.shape[0]:
        s = (mat[pi,ci_uq8]+mat[ci_uq8,pi])/2
...
for jf in glob.glob(f"{CYCLE_DIR}/af3/outputs/batch_*/id*/*summary_confidences.json"):
    ...
    if len(pc) > ci_uq8: af3[nid] = float(pc[pi][ci_uq8])
...
for nid, cif_path in af3_cif_paths.items():
    try:
        ...
        uq8_atoms = np.array([a.coord for a in structure["D"].get_atoms() if a.get_parent().resname.strip() in ["UQ8","Q8"]])
        hemc_atoms = np.array([a.coord for a in structure["C"].get_atoms() if a.get_parent().resname.strip()=="HEM"])
        if len(uq8_atoms)==0 or len(hemc_atoms)==0: continue
        ...
        burial_vs_hemc[nid] = uq8_burial/hemc_burial if hemc_burial>0 else 0.0
        fe_b=fe_c=his37=his95=his9=his67=None
        for res in structure["B"]:
            if res.resname.strip()=="HEM" and "FE" in res: fe_b=res["FE"].coord
        for res in structure["C"]:
            if res.resname.strip()=="HEM" and "FE" in res: fe_c=res["FE"].coord
        for res in structure["A"]:
            if res.id[1]==37 and "NE2" in res: his37=res["NE2"].coord
            if res.id[1]==95 and "NE2" in res: his95=res["NE2"].coord
            if res.id[1]==9 and "NE2" in res: his9=res["NE2"].coord
            if res.id[1]==67 and "NE2" in res: his67=res["NE2"].coord
        gate_b = fe_b is not None and his37 is not None and his95 is not None and \
                 np.linalg.norm(his37-fe_b)<3.0 and np.linalg.norm(his95-fe_b)<3.0
        gate_c = fe_c is not None and his9 is not None and his67 is not None and \
                 np.linalg.norm(his9-fe_c)<3.0 and np.linalg.norm(his67-fe_c)<3.0
        coord_gate[nid] = bool(gate_b and gate_c)
    except: pass
print(f"  Burial: {len(burial_vs_hemc)} Gate: {len(coord_gate)}")
...
for nid in all_ids:
    ...
    bur=burial_vs_hemc.get(nid,0.0); gate=coord_gate.get(nid,False)
    ...
    results.append({
        ...
        "burial_vs_hemc":round(bur,4),"haem_coordination_gate":gate,
        "track7_all5_mean":round((a+c+b+e+bur)/5,4)
    })

out_csv = f"{TRAJ}/all_scores_Q8_cycle{cycle}.csv"
...
print("\nTop 10 gated:")
for r in gated[:10]:
    print(f"  id{r['id']} ({r['lineage']} orig{r['original_id']}): score={r['track7_all5_mean']:.4f} boltz={r['boltz_pl']:.3f} esm3={r['esm3_ptm']:.3f} chai={r['chai_pl']:.3f} af3={r['af3_pl']:.3f} burial={r['burial_vs_hemc']:.3f}")
print(f"\nSaved to {out_csv}")
