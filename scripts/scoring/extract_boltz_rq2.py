# boltz protein-cofactor extraction for RQ2 
import json, glob, re, csv, numpy as np, sys
base, out = sys.argv[1], sys.argv[2]
def cof(pc):
    return float((pc['0']['2'] + pc['2']['0']) / 2) ## assumes 3 chain layout 
rows = {}
for d in glob.glob(f"{base}/*_id*"):
    ...
    for jf in glob.glob(f"{d}/confidence_*_model_*.json"):
        ...
        if not pc or '2' not in pc: continue ## skips designs missing 3rd chain
        v = cof(pc)
...
if rows:
    ...
    print("STILL 0 - check dir names") # failuire message 
